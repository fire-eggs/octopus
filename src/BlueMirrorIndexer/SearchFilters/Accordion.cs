...
namespace BlueMirrorIndexer.SearchFilters
{
    ...
    public class Accordion : UserControl, IMessageFilter
    {
        ...
        public static IResizeBarFactory GlobalResizeBarFactory = null; //new DefaultResizeBarFactory();
        ...
        private Control resizeBarHiding;
        private Control2 currentControl2; // used for arrow keys
        private IntPtr hwndPreviousFocus = IntPtr.Zero; // handle of window with previous focus before mouse down
        private bool ignoreNCCALCSIZE;
        private bool ignoreNCPAINT;
        //--------------------------------------------
        ...
        public CheckBox Add(AddArgs args)
        {
            ...
            if (addResizeBar.Value)
            {
                ...
                IResizeBarFactory rbf = null; //(ResizeBarFactory != null ? ResizeBarFactory : GlobalResizeBarFactory);
        ...
        private class Control2 : UserControl
        {
            ...
            //--
            internal readonly LayoutArgs layoutArgs;
            internal Control ResizeBar { get; private set; }
        ...
        public void SetTooltip(int i, string ttip)
        {
            var cb = CheckBox(i);
            tips.SetToolTip(cb, ttip);
        }
        public void SetText(int i, string txt)
        {
            var cb = CheckBox(i);
            cb.Text = (cb.Checked ? GetUpArrow() : GetDownArrow()) + txt;
        }

        public void SetText(Control who, string txt)
        {
            var cb = CheckBoxForControl(who);
            cb.Text = (cb.Checked ? GetUpArrow() : GetDownArrow()) + txt;
        }
        public void SetTooltip(Control who, string ttip)
        {
            var cb = CheckBoxForControl(who);
            tips.SetToolTip(cb, ttip);
        }
